...
for t = 0:dt:end_t
    ...
    Rwb2 = euler2Rotation2(eulerAngles);
...
PLOT = 1;
if PLOT
    figure;plot(time,pos(:,1:3));title('位置')
    figure;plot(time , pos(:,4:6)/pi*180);title('角度')
end

%% 加噪声
add_nosie = 0;
if add_nosie == 1
     sigma_acc  = 16;           % accelerometer noise (mm/s^2)
...
function RIb  = euler2Rotation2(eulerAngles)
