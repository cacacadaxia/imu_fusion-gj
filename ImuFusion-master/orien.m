...
% state.
%%请注意，我们假设IMU的偏差在整个过程中不会改变。当传感器在稳定状态下运行时，这几乎是正确的。(稳定状态运行？)
...
grt_data = data.grt_data;   % Ground truth (GT)基本事实
grt_q = grt_data(:, 5:8);   % GT quaternions GT四元数
