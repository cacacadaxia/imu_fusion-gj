...
% imu_data_tp = imu_data;
%% 加噪声
add_nosie = 1;
if add_nosie == 1
     sigma_acc  = 1;           % accelerometer noise (mm/s^2)
    sigma_acc_randwalk = 0.02;
...
% figure;plot(imu_data(1:3,:)'-imu_data_tp(1:3,:)');
%% 中值积分
gw        = [0, 0, -9.81e3]';
...
pos_measure2 = pos(:,1:3) + sigma_pos2*randn(size(pos(:,1:3)));
...
for i = 1:length(fai_measure)
    quat_measure(i,:) = oula2quat([fai_measure(i);0;0]);
end
H = [eye(3)    , zeros(3,7);
    eye(3)    , zeros(3,7);
    zeros(4,6) , eye(4)];
R =  [sigma_pos^2 * eye(3) ,zeros(3,3) , zeros(3,4);
    zeros(3,3) , sigma_pos2^2 * eye(3) , zeros(3,4);
    zeros(4,3)   , zeros(4,3) , eye(4).*[sigma_quat1;sigma_quat2;sigma_quat3;sigma_quat3].^2];
...
for k = 1:N
    ...
    w = imu_data(4:6, k); % - bias_w;
    quat = x(7:10,1);
    a = imu_data(1:3, k); % - bias_a;
...
%     Vq*quat/2 - acc + gw
    ...
    K = (P*H')/(H*P*H'+R);
    mea = [pos_measure(k,:) ,pos_measure2(k,:), quat_measure(k,:)];
...
fprintf('pos err is(mm)')
disp(std(pos(:,1:3)- x_r(1:3,:)'))
fprintf('angle err is(deg)')
disp(std(x_oula'- pos(:,4:6))/pi*180)
